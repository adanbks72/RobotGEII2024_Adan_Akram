...
double angularAccel = 3.0;        // rad/s^2
double maxLinearSpeed = 1.3;      // m/s
double minMaxLinenearSpeed = 0.5; // m/s
double linearAccel = 1.5;         // m/s^2

int current_state = IDLE;
int index = 0;      // tu veux garder index ? OK
#define MAX_POS 7
...
// Bord ? centre ? carré 1m autour ? centre
Waypoint_t waypoints[MAX_POS] = {
    {0.1, 0.2, 0},    // départ bord
    {1.0, 1.5, 0},    // centre table
    {0.5, 1.0, 0},    // carré coin bas-gauche
    {1.5, 1.0, 0},    // carré coin bas-droite
    {1.5, 2.0, 0},    // carré coin haut-droite
    {0.5, 2.0, 0},    // carré coin haut-gauche
    {1.0, 1.5, 0}     // retour centre
};
...
void UpdateTrajectory() // Mise a jour de la trajectoire en fonction de l'etat actuel par rapport au waypoint
{
    ...
    double thetaRestant = ModuloByAngle(ghostPosition.theta, thetaTarget) - ghostPosition.theta; ghostPosition.angleToTarget = thetaRestant;
